...
Uart::Uart(Port port, int tx_pin, int rx_pin, Baudrate baudrate) : _port(port)
{
    ...
    _gpio_tx = std::make_unique<Gpio>(tx_pin, Gpio::Mode::UART, Gpio::Direction::OUTPUT);
    _gpio_rx = std::make_unique<Gpio>(rx_pin, Gpio::Mode::UART, Gpio::Direction::INPUT);
...
Uart::~Uart()
{
    uart_deinit(port_to_uart(_port));
}
