...
class Impl {
 public:
  ...
  void ConfigureAndStart(uint32_t src, uint32_t dst, size_t len) {
    dma_channel_configure(_channel, &_config, reinterpret_cast<void*>(dst),
                          reinterpret_cast<const void*>(src), len, true);
  }
  ...
  void SetDreq(uint dreq) { channel_config_set_dreq(&_config, dreq); }
  ...
  bool isDone() const { return dma_channel_get_irq0_status(_channel); }
  void AckIrq() const { dma_channel_acknowledge_irq0(_channel); }
...
Dma::Dma(TransferSize size) { _impl = std::make_unique<Impl>(size); }
...
void Dma::SetTransferSize(TransferSize size) { _impl->SetTransferSize(size); }

void Dma::SetDreq(int dreq) { _impl->SetDreq(dreq); }

void Dma::SetIrq(void (*handler)(void)) { _impl->SetIrq(DMA_IRQ_0, handler); }

bool Dma::isDone() const { return _impl->isDone(); }

void Dma::AckIrq() const { _impl->AckIrq(); }
