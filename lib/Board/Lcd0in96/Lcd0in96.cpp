...
extern "C" void dma_handler_c() { Lcd0in96::dma_handler(); }
...
void Lcd0in96::SendCommand(const uint8_t* cmd, size_t cmd_size,
...
void Lcd0in96::SendColor(const uint8_t* cmd, size_t cmd_size, uint8_t* param,
                         size_t param_size) {
  _dc->Write(false);
  SendData(cmd, cmd_size);
  ...
  _dma->ConfigureAndStart(reinterpret_cast<uint32_t>(param),
                          _spi->GetTxRegisterAddr(), param_size / 2);
}
...
void Lcd0in96::InitLcdSpi() {
  _spi = std::make_unique<Spi>(LCD_SPI_PORT, LCD_CLK_PIN, LCD_MOSI_PIN, 0,
                               LCD_SPI_BAUDRATE);
}
